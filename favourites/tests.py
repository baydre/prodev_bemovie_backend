...
class FavoriteMovieAPITestCase(APITestCase):
    ...
    def test_add_favorite_movie(self, mock_get_details):
        ...
        self.assertTrue(FavoriteMovie.objects.filter(user=self.user, movie_id=1).exists())
    ...
    def test_add_duplicate_favorite_movie(self, mock_get_details):
        ...
        response = self.client.post(self.list_create_url, {'movie_id': 1}) # Try adding again
        self.assertEqual(response.status_code, status.HTTP_409_CONFLICT)

    def test_delete_favorite_movie(self):
        favorite = FavoriteMovie.objects.create(user=self.user, movie_id=1, title='Test Movie')
