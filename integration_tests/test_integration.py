...
class IntegrationTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.movie_recommendations_url = lambda movie_id: reverse('movie-recommendations', kwargs={'movie_id': movie_id})
    ...
    def test_full_workflow(self):
        ...
        with patch.object(TMDbService, 'get_movie_details', return_value=self.mock_movie_data):
            response = self.client.post(self.favorite_list_url, {
                'movie_id': self.mock_movie_data['id']
            })
            self.assertEqual(response.status_code, status.HTTP_201_CREATED)
            
            # Ensure the movie is added to favorites
            self.assertTrue(FavoriteMovie.objects.filter(user__username='integrationuser', movie_id=1).exists())
        ...
        with patch.object(TMDbService, 'get_movie_recommendations', return_value={'results': [self.mock_movie_data]}):
