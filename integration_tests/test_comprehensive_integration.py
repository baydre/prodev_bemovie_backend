...
class ComprehensiveIntegrationTestCase(APITestCase):
    ...
    def test_end_to_end_movie_app_workflow(self):
        ...
        with patch.object(
            TMDbService, "get_trending_movies", return_value=self.mock_trending_data
        ):
        ...
        with patch.object(
            TMDbService, "search_movies", return_value=self.mock_trending_data
        ):
        ...
        with patch.object(
            ...
            return_value=self.mock_trending_data,
        ):
            recommendations_url = reverse(
                "movie-recommendations", kwargs={"movie_id": 550}
            )
        ...
        favorite = FavoriteMovie.objects.get(
            user__username=self.user_data["username"], movie_id=550
        )
    ...
    def test_error_scenarios(self):
        ...
        response = self.client.get(
            reverse("favorite-movie-detail", kwargs={"pk": 99999})
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
